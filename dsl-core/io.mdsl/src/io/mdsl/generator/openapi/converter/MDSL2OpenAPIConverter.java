...
import io.mdsl.generator.OpenAPISLAGenerator;
import io.mdsl.generator.SLAConverter;
...
public class MDSL2OpenAPIConverter {
	...
	public OpenAPI convert() {
		...
		if (visibilityAndDirection != null && !visibilityAndDirection.equals("")) {
			...
			descriptionText += visibilityAndDirection; // TODO turn pattern names into plain lower case text or
														// hyperlink
		}
	...
	private String getDescription() {
		return mdslSpecification.getDescription() != null && !"".equals(mdslSpecification.getDescription())
				? mdslSpecification.getDescription()
				: null;
	}
	...
	private String getVisibilityAndDirectionInformation() {
		...
		mdslSpecification.getDirection().forEach(direction -> {
			if (directionToString(direction) != null && !"".equals(directionToString(direction)))
				result.append(directionToString(direction));
		});
	...
	private List<Tag> createTagsViaEndpointInstanceAndItsResources() {
		...
		for (HTTPResourceBinding resourceBinding : EcoreUtil2.eAllOfType(mdslSpecification,
				HTTPResourceBinding.class)) {
	...
	public Tag createTag(EndpointContract endpointType, HTTPResourceBinding resourceBinding,
			boolean createExternalDescription) {
	...
	private ExternalDocumentation createExternalDescriptionForTag(EndpointContract endpointType) {
		...
		if(!instanceList.isEmpty()) {
			...
			for (EndpointInstance instance : instanceList) {
				if(instance.getSla() != null && instance.getSla().getExternal() != null) {
	...
	private List<Tag> createTagsViaEndpointType() {
		...
		for (EndpointContract endpointType : mdslSpecification.getEndpointContracts()) {
			Tag tag = new Tag();
			tag.setName(endpointType.getName());
			// tag.setDescription("Endpoint type:" + endpointType.getName()); // does not
			// look good in some OAS tools
			ExternalDocumentation externalDocs = createExternalDescriptionForTag(endpointType);
			tag.setExternalDocs(externalDocs);
			tags.add(tag);
		}
	...
	public SecurityScheme convertPolicy2SecurityScheme(SecurityPolicy sp, SecurityBinding secBinding) {
		// expects this structure in MDSL: protected by policy
		// "HTTPBasicAuthentication": MD<string>
		...
		if (secBinding.getHttp().getValue() == OASSecurity.BASIC_AUTHENTICATION_VALUE) {
			...
			implicitOAuthObject.authorizationUrl(mdslWrapper.findAuthorizationUrlInPolicy(secBinding)); // same UR used
																										// twice now
	...
	private Paths convertEndpoints2Paths() {
		...
		for (EndpointContract endpointType : mdslSpecification.getEndpointContracts()) {
			...
			// TODO (future work) possibly also
			// https://swagger.io/specification/#callback-object (SSEs?)

			MDSLLogger.reportInformation("Endpoint type " + endpointType.getName() + " has "
					+ endpointInstanceList.size() + " HTTP binding(s).");
			...
			if (endpointInstanceList.size() == 0) {
				...
				for (int i = 0; i < endpointInstanceList.size(); i++) {
					...
					if (endpointInstanceList.get(i).getLocation().startsWith("/")) {
						MDSLLogger.reportInformation(
								"Next endpoint instance: " + endpointInstanceList.get(i).getLocation());
						...
						if (templates.size() > 0) {
							MDSLLogger.reportWarning(
									"Found one or more URI template parameters on endpoint level, not mapped.");
						}
					} else {
					...
					EList<HTTPResourceBinding> bindings = mdslWrapper
							.getHTTPResourceBindings(endpointInstanceList.get(i));
					...
					if (bindings.size() == 0) {
						...
						for (int j = 0; j < bindings.size(); j++) {
							String relURI = "";
							// TODO (future work) check that relative URI is there and makes sense (API
							// Linter?)
							...
							if (binding.getUri() != null && !binding.getUri().equals("")) {
								...
								MDSLLogger.reportWarning(
										"HTTP binding does not have a relative URI, adding resource name "
												+ endpointType.getName());
								relURI = "/" + binding.getName();
							}

							PathItem mappedEndpoint = pathsConverter.convertMetadataAndOperations(endpointType,
									binding);
	...
	// TODO (M) v55 refactor and improve error message: include value of pathURI;
	// use MDSLLogger.reportWarning and skip duplicate rather than stop
	...
	private void mergePathItems(PathItem existingPathItem, PathItem mappedEndpoint) {
		...
		if (mappedOp != null) {
			if (existingPathItem.getPost() != null)
				throw new MDSLException("Cannot add " + mappedEndpoint.getPost().getOperationId()
						+ " to path item: Path URI already defines a POST method, ignoring second one: "
						+ existingPathItem.getPost().getOperationId());
			else {
		...
		if (mappedOp != null) {
			if (existingPathItem.getGet() != null)
				throw new MDSLException("Cannot add " + mappedEndpoint.getGet().getOperationId()
						+ " to path item: Path URI already defines a GET method: "
						+ existingPathItem.getGet().getOperationId());
			else {
		...
		if (mappedOp != null) {
			if (existingPathItem.getPut() != null)
				throw new MDSLException("Cannot add " + mappedEndpoint.getPut().getOperationId()
						+ " to path item: Path URI already defines a PUT method: "
						+ existingPathItem.getPut().getOperationId());
			else {
		...
		if (mappedOp != null) {
			if (existingPathItem.getPatch() != null)
				throw new MDSLException("Cannot add " + mappedEndpoint.getPatch().getOperationId()
						+ " to path item: Path URI already defines a PATCH method: "
						+ existingPathItem.getPatch().getOperationId());
			else {
		...
		if (mappedOp != null) {
			if (existingPathItem.getDelete() != null)
				throw new MDSLException("Cannot add " + mappedEndpoint.getDelete().getOperationId()
						+ " to path item: Path URI already defines a DELETE method: "
						+ existingPathItem.getDelete().getOperationId());
			else {
		...
		if (mappedOp != null) {
			if (existingPathItem.getHead() != null)
				throw new MDSLException("Cannot add " + mappedEndpoint.getHead().getOperationId()
						+ " to path item: Path URI already defines a HEAD method: "
						+ existingPathItem.getHead().getOperationId());
			else {
		...
		if (mappedOp != null) {
			if (existingPathItem.getOptions() != null)
				throw new MDSLException("Cannot add " + mappedEndpoint.getOptions().getOperationId()
						+ " to path item: Path URI already defines a OPTIONS method: "
						+ existingPathItem.getOptions().getOperationId());
			else {
